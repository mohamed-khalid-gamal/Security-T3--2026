...
public class HillCipher {
    ...
    public List<Integer> analyse3By3Key(List<Integer> plainText, List<Integer> cipherText) {
        int n = 3;
        if (plainText.size() < n * n || cipherText.size() < n * n) {
            throw new InvalidAnalysisException();
        }

        List<Integer> P = new ArrayList<>();
        for (int i = 0; i < n * n; i++) {
            P.add(plainText.get(i));
        }

        List<Integer> C = new ArrayList<>();
        for (int i = 0; i < n * n; i++) {
            C.add(cipherText.get(i));
        }

        List<Integer> P_mat = new ArrayList<>();
        for (int row = 0; row < n; row++) {
            for (int col = 0; col < n; col++) {
                P_mat.add(plainText.get(col * n + row));
            }
        }

        List<Integer> C_mat = new ArrayList<>();
        for (int row = 0; row < n; row++) {
            for (int col = 0; col < n; col++) {
                C_mat.add(cipherText.get(col * n + row));
            }
        }

        List<Integer> P_inv = invertMatrix(P_mat, n);
        List<Integer> key = multiplyMatricesMod26(C_mat, P_inv, n);
        return key;
    }
}
